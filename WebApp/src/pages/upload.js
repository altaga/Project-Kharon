...
class Upload extends Component {
    ...
    mintNFT() {
        ...
        mint_contract.methods.mintNFT(this.props.my_ipfslink.ipfslink.nft, `[${this.state.lat},${this.state.lng}]`).send().on('transactionHash', (hash) => {
            this.unirest('POST', 'https://us-south.functions.appdomain.cloud/api/v1/web/Blankit/default/AddProductToDB')
                .headers({
                    'Content-Type': 'application/json'
                })
                .send(JSON.stringify({
                    ...
                    "url": this.props.my_ipfslink.ipfslink.nftaws,
                    "time": Math.round(Date.now()/1000),
                    'data': this.props.my_nft.nft,
                }))
                .end((res) => {
                    if (res.error) throw new Error(res.error);
                    this.setState({
                    ...
                    <div>
                        ...
                        }
                        {
                            ...
                            <div style={{ paddingTop: "12vh" }}>
                                <SimpleReactFileUpload callback={this.selectionChange} devices={this.state.devices} url={"https://api.kharonprotocol.com/upload-NFT-Storage"} />
                            </div>
                        }
                        {
